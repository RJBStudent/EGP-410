...
public:
	...
	void setSteering(Steering::SteeringType type, Vector2D targetLoc = ZERO_VECTOR2D, const float theTargetRadius = 0,
		const float theSlowRadius = 0, const float theTimeToTarget = 0.1, const float theTargetRadians = 0,
		const float theSlowRadians = 0, const float theWanderOffset = 0, const float theWanderRadius = 0,
		const float theWanderRate = 0, const float theWanderOrientation = 0, const float theChaseRadius = 0, const float theGroupRadius = 0, UnitID targetUnitID = INVALID_UNIT_ID);
